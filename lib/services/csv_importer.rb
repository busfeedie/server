...
module Services
  # This imports a gtfs csv file into the relevant classes
  class CsvImporter
    def initialize(csv_folder_name:, app:, file:, agency_id:)
      @path = csv_folder_name
      @app = app
      @file = file
    ...
    def import!
      app = @app
      Rails.logger.tagged(
        "csv_folder_name: #{@path}", "app_id: #{@app.id}", "file: #{@file}", "agency_id: #{@gtfs_agency_id}"
      ) do
        ActiveRecord::Base.transaction do
          if @file.blank? || @file == 'agency'
            Rails.logger.info("Importing data from #{@path}agency.txt")
            CSV.foreach(@path.join('agency.txt'), headers: true) do |row|
          ...
          if @file.blank? || @file == 'routes'
            ...
            CSV.foreach(@path.join('routes.txt'), headers: true) do |row|
              next if @gtfs_agency_id.present? && row['agency_id'] != @gtfs_agency_id
          ...
          if @file.blank? || @file == 'trips'
            ...
            trip_id_to_trip = {}
            trips = [].to_set
            CSV.foreach(@path.join('trips.txt'), headers: true) do |row|
          ...
          if @file.blank? || @file == 'calendar'
            ...
            CSV.foreach(@path.join('calendar.txt'), headers: true) do |row|
          ...
          if @file.blank? || @file == 'calendar_dates'
            ...
            CSV.foreach(@path.join('calendar_dates.txt'), headers: true) do |row|
              next if @gtfs_agency_id.present? && service_to_trip[row['service_id']].blank?
              ...
              calendar_date = ::CalendarDate.create!(
                ...
                gtfs_service_id: row['service_id'],
                date: row['date'],
                exception_type: row['exception_type'].to_i - 1
              )
              if service_to_trip.present?
                service_to_trip[row['service_id']]&.each do |trip|
          ...
          if @file.blank? || @file == 'shapes'
            ...
            shapes = {}
            CSV.foreach(@path.join('shapes.txt'), headers: true) do |row|
          ...
          if @file.blank? || @file == 'stop_times'
            ...
            stop_to_stop_times = {}
            CSV.foreach(@path.join('stop_times.txt'), headers: true) do |row|
          ...
          if @file.blank? || @file == 'stops'
            ...
            CSV.foreach(@path.join('stops.txt'), headers: true) do |row|
